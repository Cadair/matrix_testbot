...
def get_or_create_room(room_alias):
...
def register_user(username, password):
    ...
    resp = requests.post(f"{MATRIX_API}/register?kind=user", json={"username": username, "password": password, "auth":{"session": session, "type": "m.login.dummy"}})
...
room_ids = {name: get_or_create_room(room_alias) for name, room_alias in rooms.items()}
print(room_ids)
